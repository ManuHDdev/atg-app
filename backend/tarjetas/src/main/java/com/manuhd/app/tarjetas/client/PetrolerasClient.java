package com.manuhd.app.tarjetas.client;
 ...
 * del usuario que está operando: aquí no se monta ninguna autenticación propia.
 */
@Component
...
public class PetrolerasClient {
    ...
    /** Módulo con el que las plantillas de documento están dadas de alta en petroleras. */
    private static final String MODULO_TARJETAS = "TARJETAS";
     ...
     *     todavía no tiene plantilla para ese tipo (404), o con el error de comunicación en
     *     cualquier otro caso
     */
    public byte[] obtenerPlantillaDocumento(Long petroleraId, TipoSolicitud tipoSolicitud) throws IOException {
        String url = UriComponentsBuilder.fromUriString(petrolerasBaseUrl)
                ...
                .queryParam("modulo", MODULO_TARJETAS)
        ...
        try {
            ...
            throw new IOException("PLANTILLA_NO_CONFIGURADA: La petrolera no tiene configurada una plantilla de "
                    + tipoSolicitud.name() + " para tarjetas. Contacte con un administrador.", e);
        } catch (IOException e) {
            throw e;
        } catch (Exception e) {
